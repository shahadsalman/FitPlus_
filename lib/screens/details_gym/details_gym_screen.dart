...
class DetailsGymScreen extends StatefulWidget {
  DetailsGymScreen({super.key, required this.data, this.subscribeUser = false, required this.currentUser});
...
class _DetailsGymScreenState extends State<DetailsGymScreen> {
                          ...
                          child: ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              return Container(
                                ...
                                margin: const EdgeInsetsDirectional.only(end: 10),
                                child: Center(child: Text('${widget.data['workingHours'][index]['name']} ${widget.data['workingHours'][index]['Time']}')),
                              );
                            },
                          ),
                        ),
                                                  ...
                                                  child: Align(
                                                    ...
                                                    child: GestureDetector(
                                                      ...
                                                      child: Container(
                                                        ...
                                                        decoration: BoxDecoration(
                                                          color: const Color.fromARGB(255, 186, 173, 205),
                                                          borderRadius: BorderRadius.circular(8),
                                                        ),
  ...
  Future<void> makePayment() async {
    try {
      widget.paymentIntentData = await createPaymentIntent('', 'SAR'); //json.decode(response.body);
  ...
  saveSubscriberUser() {
    ...
    FbFireStoreController().updateArray(nameCollection: 'users', data: [widget.data.id], nameDoc: token, nameList: 'listSubscribe');
    FbFireStoreController().updateArray(nameCollection: 'users', data: [token], nameDoc: widget.data.id, nameList: 'listSubscribe');
  }
}
